		...
		function ClientViewModel() {
			...
			self.connected = function () {
				...
				self.clientNeedsTableColumns = [
					...
					{ headerText: 'Status', field: "statusDisplayed" },
					{ headerText: 'Need met?', field: "need_met", headerStyle: "text-align: center;", style: "text-align: center; padding-right: 23px;" },
							...
							.then(
								success = function (response) {
									$.each(response.client_needs, function (index, item) {
										...
										var statusDisplayed = this.fulfilling_organization_name + (this.need_met === "Y" ? " has fulfilled this need" : " has agreed to help");

										if (this.fulfilling_organization_name === null || this.fulfilling_organization_name === "") {
											if (this.pending_responses == 0) {
												statusDisplayed = "No assistance available";
											} else {
												statusDisplayed = "Waiting for responses";
											}
										}
										...
										self.clientNeedsValues().push({
											...
											id: this.id,
											need_met: this.need_met === "Y" ? "Yes" : "No",
											notes: this.notes,
